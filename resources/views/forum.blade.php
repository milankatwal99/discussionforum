...
@foreach($discussions as $discussion)
    <div class = "card mt-3">
        ...
        <span> {{ $discussion->user->name}} ({{$discussion->user->points}})</span>
            <a href = "{{route('forum.show',$discussion->id)}}" class = "btn btn-success btn-xs float-right">Watch</a>
            @if($discussion->hasBestAnswer())
                <span class = "btn btn-success btn-xs btn-pull-right">Close</span>
            @else
                <span class = "btn btn-success btn-xs btn-pull-right">Open</span>
            @endif
